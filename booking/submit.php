...
if (isset($_POST['submit'])) {
    ...
    try {
        ...
        foreach ($invoiceDetails as $key => $value) {
            ...
            if (strpos($key, "-") != 0) {
                ...
                if (preg_replace('/-\d+$/', '', $key) === 'roomTypeIdPrice') {
                    ...
                    foreach ($roomsTypeGet as $roomTypeGet) {
                        if ($roomTypeGet['isUseDeposit']) {
                            $roomType[$index]['price'] = ((($roomTypeIdPrice[1] * $roomTypeGet['depositPercent']) / 100) * $numberDay) * $invoiceDetails["quantity-" . $index];
                        } else {
        ...
        $sqlRoomType = "INSERT INTO invoiceDetails(invoiceId,roomTypeId,quantity,price)  VALUES(?, ?, ?, ?)";
