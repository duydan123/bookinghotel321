                                ...
                                if ($query->rowCount() > 0) {
                                    foreach ($results as $result) {
                                        ...
                                        <tr align="center">
                                            ...
                                            <td>
                                                <a href="products/details.php?id=<?php echo htmlentities($result['roomTypeId']); ?>"><?php echo htmlentities($result['roomtype_name']); ?></a>
                                            </td>
                                            <td>
                                                <?php echo htmlentities(number_format($result['roomtype_price'], 0, ',', '.')) . ' VND/ PHÒNG/ ĐÊM';?>
                                            </td>
                                            <td>
                                                <?php echo htmlentities(number_format($result['invoiceDetail_price'], 0, ',', '.')) . ' VND';?>
                                            </td>
                                            <td>
                                                <?php echo htmlentities($result['invoicedetail_quantity']); ?>
                                            </td>
                                            <td>
                                                <?php if ($result['isUseDeposit']) { echo '<b>'. htmlentities(number_format($result['roomtype_deposit'], 0, ',', '.')) . ' %' .'</b>'; } else { echo "trả thẳng không cọc"; } ?>
                                            </td>
                                            <td>
                                                <?php echo htmlentities($result['roomtype_roomCode']); ?>
                                            </td>
                                            <td>
